...
The definition includes a `"generator_file"` string, which can include a relative or absolute path. The value gives the location of a generator file for this benchmark job.
...
They also include a `"format"` string, which is `"CSV"` or `"parquet"` to determine the output file type. Finally, a string named `"output_filename"` sets the name of the output file. This string can include a relative or absolute path, and should include an extension to make the file's format clear.

### From external data ###

This generation file defines two columns, one of which uses an external file as a driver:

	{
	  "format": "CSV",
      "output_filename": "animals.csv",
	  "columns" : {
	    "animal_id" :
	    {
	      "generation_type": "id",
...
From the `"format"` and `"output_filename"` strings, we can tell that this JSON will cause the generator to produce a CSV file named `"animals.csv"`.
...
It's possible to generate data that's completely random, though repeatable. Let's consider this generator file:

	{
	  "format": "CSV",
      "output_filename": "join.csv",
	  "columns" : {
	    "Values" :
	    {
...
The `format` and `output_filename` strings indicate we'll create a CSV file, this time named "join.csv". 
