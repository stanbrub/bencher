...
start_time = time.time(); result = pd.merge(pd.merge(relation.to_pandas(), adjectives.to_pandas(), how='left', on='adjective_id'), animals.to_pandas(), how='left', on='animal_id')[['Values', 'adjective_name', 'animal_name']]; end_time = time.time()
print(len(result))
